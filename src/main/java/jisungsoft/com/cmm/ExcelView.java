...
public class ExcelView extends AbstractView {
    ...
    protected void buildExcelDocument(Map model, XSSFWorkbook wb, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        XSSFCell cell = null;
        Boolean showIdx = true;
        if(dataMap.get("showIdx") != null) showIdx = (Boolean) dataMap.get("showIdx"); // idx 여부
        ...
        if(dataList.size() > 0){ // 저장된 데이터가 있을때
            ...
            for (int i = 0; i<dataList.size(); i++) {
                ...
                if(showIdx) dataEgovMap.put("idx", (i+1)+"");

                for (int j = 0; j < columnVarArr.length; j++) {
                    String data = "";
                    if(dataEgovMap.get(columnVarArr[j]) == null){
                        data = "";
                    }else {
                        data = dataEgovMap.get(columnVarArr[j]).toString();
                    }
