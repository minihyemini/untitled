 ...
 */
package jisungsoft.com.cmm.hwp.v5;

import jisungsoft.com.cmm.hwp.utils.HwpStreamReader;
...
public abstract class HwpTextExtractorV5 {
	 ...
	 * @throws IOException
	 */
	public static boolean extractText(File source, Writer writer) throws FileNotFoundException, IOException {
		if (source == null)
	...
	public static InputStream createDecryptStream(InputStream input, Key key) throws IOException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException {
		Cipher cipher = null;
	...
	private static void extractText(HwpStreamReader sectionStream, Writer writer) throws IOException {
		...
		while (true) {
			...
			if (HWPTAG_BEGIN + 50 == tag.id) {
				writeParaHeader(sectionStream, tag.length, buf, writer);
	...
	private static void writeParaHeader(HwpStreamReader sectionStream, long length, StringBuffer buf, Writer writer) throws IOException {
		if (i > 1) {
