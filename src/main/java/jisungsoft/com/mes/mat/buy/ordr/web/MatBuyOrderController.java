package jisungsoft.com.mes.mat.buy.ordr.web;
...
@Slf4j
@Controller
@RequestMapping(value = "/mes/mat/buy/ordr")
public class MatBuyOrderController {
    @Autowired
    ...
    private final String VIEW_PATH = "jisungsoft/com/mes/mat/buy/oder";
    private final String REDIRECT_PATH = "redirect:/mes/mat/buy/ordr";
    private final String ORDER_TYPE = OrderType.PLACE.name();
    ...
    public Order newMatBuyOrderForm() {
        Order order = new Order();
        //주문타입 - 발주
        order.setOrdType(ORDER_TYPE);
        return order;
    }

    @RequestMapping(value = "/list.do", method = {RequestMethod.POST, RequestMethod.GET})
    public String list(@ModelAttribute("searchForm") Order searchForm, final Model model, Locale locale, final HttpServletRequest request) {
        ...
        searchForm.setOrdType(ORDER_TYPE);
    ...
    public String removeAction(@Validated @ModelAttribute("orderForm") Order form,
                               final BindingResult bindingResult, Model model, Locale locale,
                               final RedirectAttributes attr) {

        try {
            orderService.removeRequestOrderItem(form);
