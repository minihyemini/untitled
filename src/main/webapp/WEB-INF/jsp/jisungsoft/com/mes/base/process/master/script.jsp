...
<script>
    $(document).ready(function() {
        ...
        $('#dt-basic tbody').on('click', 'tr', function(event) {
            ...
            $.ajax({
                type:"post",
                async:false,
                url:"<c:url value='/mes/base/process/master/detail.json'/>",
                data:{'code' : id},
                dataType:'json',
    ...
    function fn_dataAction(obj) {
        ...
        let $form = document.detailCodeForm;
        if (target == 'save') {
            $form.action = '<c:url value='/mes/base/process/master/add.do'/>';
        } else if (target == 'delete') {
            $form.action = '<c:url value='/mes/base/process/master/remove.do'/>';
        }
        $form.submit();
    }
